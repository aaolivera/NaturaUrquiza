        ...
        public static MvcHtmlString IconContainer(this HtmlHelper html, ListaPaginada<ProductoDto> items)
        {
            ...
            foreach (var entidad in items)
            {
                ...
                htmlstring += @"<div class='col-lg-4'>
                                  ...
                                  <p class='text-center'><a class='btn btn-default btn-xs ajax-ver-link' href='" + urlHelper.Action("Ver", "Natura", new { id = entidad.Id }) + "' role='button'><os-p>detalles</os-p></a><a class='btn btn-default btn-xs' style='padding-bottom: 1px;' data-bind=\"click: agregarProducto.bind($data, '" + entidad.Id + "', '" + entidad.Nombre + "', '" + entidad.Precio + "', '" + entidad.PrecioPromocional + "')\" role='button' href='#'><i class='glyphicon glyphicon-shopping-cart'></i><span class='jewelCount hide' id='" + entidad.Id + "-jewel'></p></a></div>";
